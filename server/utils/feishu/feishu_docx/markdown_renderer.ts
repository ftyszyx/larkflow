...
import { escapeHTMLTags, Renderer, trimLastNewline } from './renderer.ts';
import { Buffer } from './string_buffer.ts';
import { Block, BlockType, CodeLanguage, DocMetaInfo, getAlignStyle, getCodeLanguage, ImageBlock, TableBlock, TableMergeInfo, TextBlock, TextElement, TextRun } from './types.ts';

// NOTE: In this monorepo we may have multiple `marked` installs (hoisted + package-local),
// which can cause TypeScript to treat MarkedExtension types as incompatible.
// Runtime is fine; cast to avoid TS type identity issues.
...
export class MarkdownRenderer extends Renderer {
  ...
  override parseBlock(block: Block, indent: number) {
    this.indent = indent;
   ...
   * @returns
   */
  parsePageMeta(block: Block): boolean {
    ...
    try {
      if (language === CodeLanguage.YAML) {
        this.meta = YAML.load(code) as DocMetaInfo;
      } else if (language === CodeLanguage.JSON) {
        this.meta = JSON.parse(code);
      }
      this.meta = this.meta || {} as DocMetaInfo;
      this.meta.hide = this.meta.hide || false;
      this.meta.hide_children = this.meta.hide_children || false;
    } catch {
  ...
  parseBulletBlock(block: Block, indent: number = 0): Buffer | string {
    ...
    if (this.nextBlock?.block_type == block.block_type &&
    ...
    parent?.children?.forEach((childId, idx) => {
      if (childId == block.block_id) {
        for (let i = idx - 1; i >= 0; i--) {
          if (this.blockMap[parent.children[i]].block_type ==
            BlockType.Ordered) {
            order++;
          } else {
            break;
  ...
  parseCallout(block: Block): Buffer | string {
    ...
    if (block.callout.emoji_id) {
      ...
      buf.write(getEmojiChar(String(block.callout.emoji_id)));
      buf.writeln('</div>');
    }
    ...
    this.withSubIndent(() => {
      markdownBuf.write(block.children
        ?.map((childId) => {
          const child = this.blockMap[childId];
          return this.parseBlock(child, 0);
        })
  ...
  tableCellAttrHTML(mergeInfos: TableMergeInfo[], idx: number): string {
    ...
    let html =
      Object.keys(attr).map((key) => `${key}="${attr[key]}"`).join(' ');
